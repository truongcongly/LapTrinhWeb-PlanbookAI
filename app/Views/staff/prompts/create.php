            ...
            <form method="POST" action="/LapTrinhWeb-PlanbookAI/public/staff/prompts/store">
                <div class="row g-4">
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control rounded-4 prompt-description-input" name="description" rows="3"></textarea>
                    </div>

                    <div class="col-12">
                        <label class="form-label fw-semibold">Prompt Content</label>
                        <textarea class="form-control rounded-4 prompt-content-input" name="prompt_content" rows="10" required></textarea>
                    </div>
                </div>
...
- Các phương án nhiễu phải hợp lý và không quá dễ loại trừ.`
    ...
    function syncCategory(forceFill = false) {
        ...
        if (forceFill || !descriptionInput.value.trim()) {
            descriptionInput.value = defaults[category].description;
        }

        if (forceFill || !promptInput.value.trim()) {
